function [inside_s_jmt] = base_shares_RC(p,x_jmt,xi_jmt,sigmaa,index,index2,index3)

global beta alpha Total J M T I base_heterogeneity
% p   = normrnd(5,1,Total-M*T,1);

delta_jmt          = zeros(Total,I);                                          % This will be filled with mean market values
delta_jmt(index,:) = 0;
...
for k = 1:M*T
  ...
  + repmat(sigmaa(1),J-1,1).*base_heterogeneity(1,i) - p(index3(k,1):index3(k,2),:).*sigmaa(2).*base_heterogeneity(2,i);   
    end
end
