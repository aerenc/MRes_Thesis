function [inside_s_jmt] = shares(p,x_jmt,xi_jmt,index,index2,index3)

global beta alpha Total J M T

aa = ones(Total-M*T-1,1);

delta_jmt           = zeros(Total-1,1);                                    % This will be filled with mean market values
delta_jmt(index,:)  = 0;

delta_jmt(index2(1,1):index2(1,2),:)  = [ones(J-2,1) x_jmt(index3(1,1):index3(1,2),:)] * beta - alpha .* p(index3(1,1):index3(1,2),:) + xi_jmt(index3(1,1):index3(1,2),:);                              
