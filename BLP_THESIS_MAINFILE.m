...
global x_jmt beta alpha xi_jmt Total J M T index index2 index3 inits adj m_x_jmt m_xi_jmt m_index m_index2 m_index3 m2_x_jmt m2_xi_jmt

rng('default');

%% SIMULATION SETTING - Generating Unmanipulated Data
...
%% UNMANIPULATED DATA IN 3D-ARRAY FORM:
...
%% DATA MANIPULATION & RE-ESTIMATION:

adj       = 5;                                        % 5 goods will be removed from each market & time period randomly as below:
...
m_drop_index = zeros(adj,M*T);                        % these elements will be dropped

for i = 1: M*T
m_drop_index(:,i) = randperm(J-1,adj)';
end

% Now, we reshape the data into (J-1) * (M*T) matrices for each element of
% x and w
...
mm2_drop_index = zeros(adj,M);                        % these elements will be dropped, now the dropped brands will be same for all time periods for each market

for i = 1 : M
mm2_drop_index(:,i) = randperm(J-1,adj)';
end
mm2_drop_index = [16,18,6,13,4,14,15,16,11,12,5,13,12,10,9,9,7,2,12,15;12,16,12,16,13,8,1,11,10,3,18,5,2,11,2,7,9,5,17,7;2,6,13,3,14,9,10,10,1,5,1,16,15,19,3,5,18,1,16,6;6,17,18,1,16,6,14,8,18,15,8,15,7,8,15,14,8,16,11,13;19,5,10,17,12,17,2,1,8,14,13,12,19,1,11,6,5,3,18,11];
mm2_drop_index = sort(mm2_drop_index);

m2_drop_index = zeros(adj,M*T);
...
for i = 1 : M
    m2_drop_index(:,i:M:M*T-M+i)  =     repmat(mm2_drop_index(:,i),1,10);
end

% Now, we reshape the data into (J-1) * (M*T) matrices for each element of
% x and w

m2reshaped_x1_jmt = reshape(x_jmt(:,1),J-1,M*T); 
m2reshaped_x2_jmt = reshape(x_jmt(:,2),J-1,M*T); 
m2reshaped_x3_jmt = reshape(x_jmt(:,3),J-1,M*T); 
m2reshaped_w1_jmt = reshape(w_jmt(:,1),J-1,M*T); 
m2reshaped_w2_jmt = reshape(w_jmt(:,2),J-1,M*T); 
m2reshaped_w3_jmt = reshape(w_jmt(:,3),J-1,M*T); 

m2reshaped_xi_jmt    = reshape(xi_jmt,J-1,M*T); 
m2reshaped_omega_jmt = reshape(omega_jmt,J-1,M*T);

for  i = 1 : M*T
    m2reshaped_x1_jmt(m2_drop_index(:,i),i) = 0;
end
for  i = 1 : M*T
    m2reshaped_x2_jmt(m2_drop_index(:,i),i) = 0;
end
for  i = 1 : M*T
    m2reshaped_x3_jmt(m2_drop_index(:,i),i) = 0;
end
for  i = 1 : M*T
    m2reshaped_w1_jmt(m2_drop_index(:,i),i) = 0;
end
for  i = 1 : M*T
    m2reshaped_w2_jmt(m2_drop_index(:,i),i) = 0;
end
for  i = 1 : M*T
    m2reshaped_w3_jmt(m2_drop_index(:,i),i) = 0;
end
for  i = 1 : M*T
    m2reshaped_xi_jmt(m2_drop_index(:,i),i) = 0;
end
for  i = 1 : M*T
    m2reshaped_omega_jmt(m2_drop_index(:,i),i) = 0;
end

%%
% m2_ stands for manipulated for our second case, i.e. when randomly chosen equal number (#: 5) of brands are exempted from all time periods of a given market:

m2_x1_jmt = m2reshaped_x1_jmt(:);
m2_x1_jmt = m2_x1_jmt(m2_x1_jmt~=0);
m2_x2_jmt = m2reshaped_x2_jmt(:);
m2_x2_jmt = m2_x2_jmt(m2_x2_jmt~=0);
m2_x3_jmt = m2reshaped_x3_jmt(:);
m2_x3_jmt = m2_x3_jmt(m2_x3_jmt~=0);

m2_x_jmt = [m2_x1_jmt m2_x2_jmt m2_x3_jmt] ;

m2_w1_jmt = m2reshaped_w1_jmt(:);
m2_w1_jmt = m2_w1_jmt(m2_w1_jmt~=0);
m2_w2_jmt = m2reshaped_w2_jmt(:);
m2_w2_jmt = m2_w2_jmt(m2_w2_jmt~=0);
m2_w3_jmt = m2reshaped_w3_jmt(:);
m2_w3_jmt = m2_w3_jmt(m2_w3_jmt~=0);

m2_w_jmt = [m2_w1_jmt m2_w2_jmt m2_w3_jmt];

m2_xi_jmt = m2reshaped_xi_jmt(:);
m2_xi_jmt = m2_xi_jmt(m2_xi_jmt~=0);

m2_omega_jmt = m2reshaped_omega_jmt(:);
m2_omega_jmt = m2_omega_jmt(m2_omega_jmt~=0);

m2_mc_jmt = [ones(Total-M*T-adj*M*T,1) m2_w_jmt] * gamma + m2_omega_jmt; % getting marginal costs for manipulated data

m2_inits               = ones(Total-M*T-adj*M*T,1);
m2_solveforprices      = @(pp) (pp -(1./(alpha.*(1 - m2_s_jmt(pp)))) - m2_mc_jmt);
%opts                  = optimset('Algorithm ','interior-point ','Display ','iter ');
%opts                  = optimset('Display','off');
opts                   = optimset('Display','iter','TolCon',1E-8,'TolFun',1E-8,'TolX',1E-10,'MaxFunEvals',1000000000); %

tic
m2_p_jmt               = fsolve(m2_solveforprices,m2_inits,opts); % this gives us prices
toc

m2_s_jmt               = m2_s_jmt(m2_p_jmt);          % gives us inner market shares
cumsum_m2_s_jmt        = cumsum(m2_s_jmt);
cumsum_m2_s_jmt_1      = cumsum_m2_s_jmt(J-adj-1,:);
marketwisesum_m2_s_jmt = diff(cumsum_m2_s_jmt(m_indexx,:)); 
marketwisesum_m2_s_jmt = [cumsum_m2_s_jmt_1;marketwisesum_m2_s_jmt]; 
m2_s0_jmt              = 1 - marketwisesum_m2_s_jmt;
